...
| 2026-05-20 | 1.1.1 | DB schema: `session_operations.question_correct_answer` BIGINT→JSON, `user_answer` INT→JSON, `operation_type VARCHAR(50)` column added (NULL for Quick Calculate). No logic changes. | 🔄 In Progress |
...
2. **Compare Answers** - Cast `user_answer` (JSON) to int and compare with `int(correct_answer)` — both columns are JSON type; explicit `int()` cast required
